...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.home_image = customtkinter.CTkImage(light_image=Image.open(os.path.join(image_path, "ic_home_dark.png")),
                                                 dark_image=Image.open(os.path.join(image_path, "ic_home.png")), size=(20, 20))
        self.chat_image = customtkinter.CTkImage(light_image=Image.open(os.path.join(image_path, "ic_creation_dark.png")),
                                                 dark_image=Image.open(os.path.join(image_path, "ic_creation.png")), size=(20, 20))
        self.add_user_image = customtkinter.CTkImage(light_image=Image.open(os.path.join(image_path, "ic_exposition_dark.png")),
                                                     dark_image=Image.open(os.path.join(image_path, "ic_exposition.png")), size=(20, 20))
        ...
        l1=customtkinter.CTkLabel(master=self.home_frame, text="Home Page",font=('Century Gothic', 60))
        ...
        for i in  range(0, len(self.creation_list)):
            for j in range(0, 4):
                ...
                lf_grocery1.grid(row=i,column=j,padx=20, pady=20,ipadx=100, ipady=20)

                grocery1_image=ptk.PhotoImage(p.Image.open("static/Images\{}".format(self.creation_list[i].image)))
                label_grocery_1=customtkinter.CTkLabel(lf_grocery1, image=grocery1_image).grid(row=0,column=1)

                name_grocery1=customtkinter.CTkLabel(lf_grocery1,text=self.creation_list[i].name).grid(row=1,column=1)
                label_qty_grocery1= customtkinter.CTkLabel(lf_grocery1,text="Qty:").grid(row=2, column=1)
                add_grocery1=customtkinter.CTkButton(lf_grocery1, text="Ajouter au panier", command=lambda i=i: self.get_current_creation(i)).grid(row=3, column=0)
        # create third frame
    ...
    def get_current_creation(self,index):
        ...
        creation = CreationDetails(creation=responce,master=self)
        creation.grid(row=0, column=1, sticky="nsew")
